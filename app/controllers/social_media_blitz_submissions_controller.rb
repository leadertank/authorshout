class SocialMediaBlitzSubmissionsController < ApplicationController
  def new
    @awards_submission = build_submission_from_token || AwardsSubmission.new(form_key: AwardsSubmission::SOCIAL_BLITZ_FORM_KEY)
  end

  def create
    @awards_submission = AwardsSubmission.new(awards_submission_params.merge(form_key: AwardsSubmission::SOCIAL_BLITZ_FORM_KEY))
    ...
    unless social_blitz_checkout_configured?
      flash.now[:alert] = "Social Media Book Blitz checkout is not configured yet. Add STRIPE_SOCIAL_BLITZ_PRICE_ID and STRIPE_SECRET_KEY."
      render :new, status: :unprocessable_entity
      return
    end
    ...
    checkout_session = Stripe::Checkout::Session.create(
      ...
      line_items: [
        {
          price: social_blitz_price_id,
          quantity: 1
        }
      ],
      customer_email: @awards_submission.author_email,
      success_url: social_media_blitz_submission_success_url(
        token: @awards_submission.public_token,
        session_id: "{CHECKOUT_SESSION_ID}"
      ),
      cancel_url: new_social_media_blitz_submission_url(token: @awards_submission.public_token),
      metadata: {
        awards_submission_token: @awards_submission.public_token,
        form_name: AwardsSubmission::FORM_LABELS.fetch(AwardsSubmission::SOCIAL_BLITZ_FORM_KEY),
        form_key: AwardsSubmission::SOCIAL_BLITZ_FORM_KEY
      }
    )

    @awards_submission.update!(stripe_checkout_session_id: checkout_session.id)
    @checkout_url = checkout_session.url
    render "awards_submissions/checkout_redirect", layout: false
  rescue StandardError => error
    Rails.logger.error("Social blitz submission checkout failed: #{error.class}: #{error.message}")
  ...
  def success
    ...
    if session_id.blank?
      redirect_to new_social_media_blitz_submission_path(token: @awards_submission.public_token), alert: "Checkout was not completed yet."
      return
    end
    ...
    redirect_to new_social_media_blitz_submission_path(token: @awards_submission.public_token), alert: "Payment was not completed. Please try again."
  rescue ActiveRecord::RecordNotFound
    redirect_to new_social_media_blitz_submission_path, alert: "Submission not found. Please complete the form again."
  rescue StandardError => error
    Rails.logger.error(
      "Social blitz submission success handling failed: #{error.class}: #{error.message}; " \
      "token=#{params[:token]} session_id=#{params[:session_id]}"
    )
    redirect_to new_social_media_blitz_submission_path, alert: "We could not verify payment right now. Please contact support if you were charged."
  end
  ...
  def build_submission_from_token
    ...
    AwardsSubmission.new(
      ...
      teaser_blurb: saved.teaser_blurb,
  ...
  def social_blitz_checkout_configured?
    social_blitz_price_id.present? && stripe_secret_key.present?
  end
